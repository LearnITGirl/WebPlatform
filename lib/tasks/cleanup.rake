namespace :cleanup do
  ...
  task applications: :environment do
    stale_mentee_applications = MenteeApplication.where(created_at: (6.months.ago..(DateTime.current - 24.hours))).where.not(build_step: "done")
    stale_mentee_applications.map(&:destroy)

    stale_mentor_applications = MentorApplication.where(created_at: (6.months.ago..(DateTime.current - 24.hours))).where.not(build_step: "done")
