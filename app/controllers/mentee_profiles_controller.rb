class MenteeProfilesController < UsersController
  ...
  def missing_mentor
    ...
    if @mentor.update_all(is_missing: true, missing_since: DateTime.current)
      MissingPersonsMailer.missing_mentor(@user).deliver_now
      send_email_to_missing_mentor
      redirect_to dashboard_mentee_profiles_path, notice: 'Organisers have been notified about the missing mentor'
    else
